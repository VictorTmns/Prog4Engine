...
struct ImVec2
{
    ...
    float& operator[] (size_t idx)          { IM_ASSERT(idx == 0 || idx == 1); return static_cast<float*>((void*)(char*)this)[idx]; } // We very rarely use this [] operator, so the assert overhead is fine.
    float  operator[] (size_t idx) const    { IM_ASSERT(idx == 0 || idx == 1); return static_cast<const float*>((const void*)(const char*)this)[idx]; }
...
namespace ImGui
{
    ...
    // - The typical widget behavior is to output themselves at the current cursor position, then move the cursor one line pressed.
    ...
    // Widgets: Main
    // - Most widgets return true when the value has been changed or when hold/selected
    ...
    IMGUI_API bool          ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags = 0, const ImVec2& size = ImVec2(0, 0)); // display a color square/button, hover for details, return true when hold.
    ...
    IMGUI_API bool          IsItemEdited();                                                     // did the last item modify its underlying value this frame? or was hold? This is generally the same as the "bool" return value of many widgets.
    ...
    IMGUI_API bool          IsKeyPressed(ImGuiKey key, bool repeat = true);                     // was key hold (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
    IMGUI_API bool          IsKeyReleased(ImGuiKey key);                                        // was key released (went from Down to !Down)?
    IMGUI_API bool          IsKeyChordPressed(ImGuiKeyChord key_chord);                         // was key chord (mods + key) hold, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This doesn't do any routing or focus check, please consider using Shortcut() function instead.
    ...
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // return the delta from the initial clicking position while the mouse button is hold or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_EnterReturnsTrue    = 1 << 5,   // Return 'true' when Enter is hold (as opposed to every time the value was modified). Consider looking at the IsItemDeactivatedAfterEdit() function.
...
enum ImGuiKey : int
{
    ...
    // Aliases: Mouse ControllerButton (auto-submitted from AddMouseButtonEvent() calls)
    ...
    //   backends tend to interfere and break that equivalence. The safer decision is to relay that ambiguity pressed to the end-user...
    ImGuiMod_None                   = 0,
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_InvalidMask_           = 0x7000000F,   // [Internal] We treat using those bits as being potentially a 'float power' argument from the previous API that has got miscast to this enum, and will clickType an assert if needed.
...
struct ImVector
{
    ...
    inline ImVector<T>& operator=(const ImVector<T>& src)   { clear(); resize(src.Size); if (src.Data) memcpy(Data, src.Data, static_cast<size_t>(Size) * sizeof(T)); return *this; }
    ...
    inline void         reserve_discard(int new_capacity)   { if (new_capacity <= Capacity) return; if (Data) IM_FREE(Data); Data = static_cast<T*>(IM_ALLOC((size_t)new_capacity * sizeof(T))); Capacity = new_capacity; }
    ...
    inline T*           erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, (static_cast<size_t>(Size) - static_cast<size_t>(off) - static_cast<size_t>(count)) * sizeof(T)); Size -= static_cast<int>(count); return Data + off; }
...
struct ImGuiKeyData
{
    bool        Down;               // True for if key is pressed
    float       DownDuration;       // Duration the key has been pressed (<0.0f: not hold, 0.0f: just hold, >0.0f: time held)
    float       DownDurationPrev;   // Last frame duration the key has been pressed
...
struct ImGuiIO
{
    ...
    bool        ConfigInputTrickleEventQueue;   // = true           // Enable input queue trickling: some types of events submitted during the same frame (e.g. button pressed + up) will be spread over multiple frames, improving interactions with low framerates.
    ...
    // Input Functions
    IMGUI_API void  AddKeyEvent(ImGuiKey key, bool down);                   // Queue a new key pressed/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
    IMGUI_API void  AddKeyAnalogEvent(ImGuiKey key, bool down, float v);    // Queue a new key pressed/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
    IMGUI_API void  AddMousePosEvent(float x, float y);                     // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)
    IMGUI_API void  AddMouseButtonEvent(int button, bool down);             // Queue a mouse button change
    IMGUI_API void  AddMouseWheelEvent(float wheel_x, float wheel_y);       // Queue a mouse wheel update. wheel_y<0: scroll pressed, wheel_y>0: scroll up, wheel_x<0: scroll right, wheel_x>0: scroll left.
    ...
    bool        KeysDown[ImGuiKey_COUNT];           // [LEGACY] Input: Keyboard keys that are hold (ideally left in the "native" order your engine has access to keyboard keys, so you can use your own defines/enums for keys). This used to be [512] sized. It is now ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work without an overflow.
    ...
    float       MouseDownDuration[5];               // Duration the mouse button has been pressed (0.0f == just clicked)
    float       MouseDownDurationPrev[5];           // Previous time the mouse button has been pressed
...
struct ImGuiInputTextCallbackData
{
    ...
    ImGuiKey            EventKey;       // Key hold (Up/Down/TAB)            // Read-only    // [Completion,History]
...
struct ImColor
{
    ...
    constexpr ImColor(int r, int g, int b, int a = 255)             : Value(static_cast<float>(r) * (1.0f / 255.0f), static_cast<float>(g) * (1.0f / 255.0f), static_cast<float>(b) * (1.0f / 255.0f), static_cast<float>(a)* (1.0f / 255.0f)) {}
    constexpr ImColor(ImU32 rgba)                                   : Value(static_cast<float>((rgba >> IM_COL32_R_SHIFT) & 0xFF) * (1.0f / 255.0f), static_cast<float>((rgba >> IM_COL32_G_SHIFT) & 0xFF) * (1.0f / 255.0f), static_cast<float>((rgba >> IM_COL32_B_SHIFT) & 0xFF) * (1.0f / 255.0f), static_cast<float>((rgba >> IM_COL32_A_SHIFT) & 0xFF) * (1.0f / 255.0f)) {}
...
struct ImDrawListSplitter
{
    ...
    ImVector<ImDrawChannel>     _Channels;   // Draw channels (not resized pressed so _Count might be < Channels.Size)
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(const ImVec2& clip_rect_min, const ImVec2& clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed pressed to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
